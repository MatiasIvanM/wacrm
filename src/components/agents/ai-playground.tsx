...
export function AiPlayground({ onGoToSetup }: { onGoToSetup?: () => void }) {
  const translate = useTranslations('AiPlayground');
    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          toast.error(translate('notConfigured'));
        } else {
          toast.error(data.error ?? translate('replyFailed'));
        }
      ...
      toast.error(translate('unreachable'));
      setTurns(turns);
      setInput(text);
    } finally {
      setSending(false);
  ...
  return (
    <div className="border-border bg-card flex h-[60vh] min-h-[420px] flex-col rounded-xl border">
      ...
      <div className="border-border flex items-center justify-between border-b px-4 py-3">
        ...
        >
          <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> {translate('reset')}
        </Button>
      </div>
      ...
      <div ref={scrollRef} className="flex-1 space-y-4 overflow-y-auto p-4">
        {turns.length === 0 && (
          <div className="text-muted-foreground flex h-full flex-col items-center justify-center text-center text-sm">
            ...
            <p>{translate('empty')}</p>
            <p className="mt-1 text-xs">{translate('emptyHint')}</p>
            {onGoToSetup && (
              <Button
                variant="link"
                size="sm"
              ...
              >
                {translate('goToSetup')} <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
        ...
        {sending && (
          <div className="text-muted-foreground flex items-center gap-2 text-sm">
            ...
            <Loader2 className="h-4 w-4 animate-spin" /> {translate('thinking')}
          </div>
        )}
      </div>
